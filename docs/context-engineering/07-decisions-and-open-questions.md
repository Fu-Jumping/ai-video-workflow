...
### D10 对话引导板块的口径与载体分层 `[临时决策]` 2026-08-30

建立"新手保障与对话引导"板块（完整设计见 [09-conversation-guidance.md](./09-conversation-guidance.md)），用户确认四项决策：**(1) 口径尽量统一**——话术规范唯一权威在 09 文档，两侧载体（根 AGENTS.md/CLAUDE.md、CLI 生成的创作项目 AGENTS.md）只摘录执行，`> **【当前】** …` / `> **【下一步】** …` 块引用两行格式逐字一致并由 `sync.test.ts` 断言钉住；**(2) 分层载体**——根 AGENTS.md 遵循 D4 只放几行行为指令，完整设计下沉 09 文档；**(3) 先写规则后补测**——行为层 E2E 验证后置，登记为 Q16；**(4) 每轮引导**——不止首触，每个实质性回复末尾附状态脚注，格式与停用条件见 09 §2。同日两次增补：脚注由单行改为**块引用两行**（用户可读性反馈：长句挤一行不便观看，每行只说一件事）；新增**偏离与非标准路线**对话口径（校验失败分类处置、偏离状态入脚注、A10 路由），详见 09 §5。

### D11 提交信息语言回归简体中文 `[通用规则]` 2026-09-03

用户裁定：git 提交信息的标题与正文一律使用**简体中文**，不再沿用 2026-08-19 后形成的英文惯例；历史提交不追溯改写。执行口径：保留 conventional 类型前缀（`feat:` / `fix:` / `docs:` / `chore:` 等，可带 scope）、代码标识符、文件路径与追踪编号（Q/H/D/F）原样，如 `fix(libtv): 为项目删除增加二次确认 (Q10)`。载体：`CONTRIBUTING.md` Rules 节与根 `AGENTS.md` 全局规则各一行；本决策登记后生效，此后新提交按此执行。
...
### Q16 ~~对话引导行为尚未经隔离 E2E 验证~~ → 已关闭 2026-08-30

- 现象：D10 建立的对话引导板块（首触引导、每轮状态脚注、初始化护航）只有静态载体与 `sync.test.ts` 生成文本断言，未验证智能体在真实对话中是否真的输出引导。
- 证据：隔离 E2E（被测快照 c07e2d1，裸镜像模拟已发布远端）：`G:\develop-G\tests\avw-guidance-e2e-20260830\REPORT.md`（4 场景逐字回复）+ `REVIEW.md`（主环境断言×证据终审）。4/4 场景通过：首触定位与选项、拒绝在工具仓库写剧本并转 init、交接三步与第一句话模板、项目内脚注与 verify 跨步提醒；脚注格式在 4 个回复末尾逐字命中。
- 影响：无残留——行为级验证与静态断言双重确认。
- 处置：已关闭，评级"可用"。偏差两项（沿用预建克隆、未实测全局安装路径）定级提示，登记于 REVIEW.md。工作位置口径补强（车间/菜模型 + 交接三步 + 步骤目录映射）已随本轮并入 09 §2 与各载体。
