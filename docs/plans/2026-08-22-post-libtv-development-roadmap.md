...
- 制定日期：2026-08-22
- 状态：草案（待确认优先级后逐项实施；2026-08-22 已确认 `lib-image-2` = gpt-image-2、新增平台枚举 `gpt-image-2`、精修分支不限于 MJ 首版）
...
3. 补齐双语文档与路线图
4. LibTV 生成执行链路产品化
5. 图片精修分支流程专项设计：任意已生成图片 + GPT Image 2 精修（可触发的分支，流程级）

其中 1、2、3 不触发生成、风险低，可先做。4、5 涉及真实生成与出图流程，而且 **5 会直接影响流程本身**：gpt-image-2 在 LibTV 中的模型身份已确认为 `lib-image-2`；5 的精修流程定位为“用户不满意时触发的分支”，不是强制二段流水线。先定稿分支状态机，再在 4 中写生成执行代码。
...
> 依赖说明：本节的生成代码实现应在第 5 节图片精修分支流程关键决策定稿后进行；本节只先列出能力目标与安全边界。
...
## 5. 图片精修分支流程专项设计：任意已生成图片 + GPT Image 2 精修

> 本项是流程级设计，不是简单模板补字段。第 4 项“LibTV 生成执行链路产品化”的代码实现应在本项关键决策定稿后进行。
>
> 流程设计初稿见 [图片精修分支流程设计 v0.1](./2026-08-22-two-stage-image-refine-flow-design.md)。
...
- 精修分支可以设计成 LibTV 画布中的“首版 image 节点 → 精修 image 节点 → 最终采用节点/版本”链路，与现有素材 adapter 边界一致。
...
当前实现假设一个关键帧只有一个 `modelKey`、一次生成、一个 `status`。精修分支会改变以下结构：
...
- 流程定位：精修是**可触发的分支**，任何已生成图片（Step 4 关键帧、Step 2 锚点图等）用户不满意 / 效果不好 / 需要修改时进入；满意则直接采用。
- 默认保护约束：核心强调“不得改动除用户指定修改点之外的任何画面区域”。
- 多轮精修基准：按用户反馈做状况识别——效果不好需要重新搞 → 用首版节点；基于当前轮继续微调 → 用当前轮精修节点。
...
- 产物与状态落位：
  - 源层 Step 4 只写“精修配置 / 精修负面约束”等可选的静态意图；
  - 执行层 `.libtv/state.json` 记录用户反馈、精修指令和动态决策，不单独新建 review 文件，不自动改写 Step 源文件。
  - 精修节点直接放在首版节点旁边，同一父分组下，不单独建组。
...
- `libtv plan/apply/status/verify-order` 表达首版/精修分支状态。
...
人工审阅（分支触发点）：
  -> 满意：标记 direct_approved，分支结束，进入视频引用链
  -> 不满意 / 效果不好 / 需要修改：标记 needs_refine，进入 GPT Image 2 精修分支
  -> 首版方向错误：标记 regenerate，用首版平台重新生成

libtv refine --allow-generation
  -> 创建精修节点，放在首版节点旁边，left 边引用首版节点
  -> 用 LibTV gpt-image-2（lib-image-2）执行中文编辑指令
...
- 流程定位已确认：可触发的分支，用户不满意 / 效果不好 / 需要修改时进入；满意则直接采用。
- `template` 风格模板：v1 默认不做。
- 用户反馈记录：写入 `.libtv/state.json`。
- 精修节点位置：直接放在原图节点旁边，同一父分组下。
- 精修节点应新建节点，还是更新原节点并保留版本？（设计初稿建议新建精修节点）
- Step 2 锚点图：已确认复用该精修分支；精修后最终采用版本进入下游引用链。
- 同一张图连续多轮精修：已确认按用户反馈做状况识别；效果不好重新搞 → 继续用首版节点，基于当前轮继续 → 引用当前轮精修节点。
...
6. 首版与精修负面约束：已确认分开维护。
7. 流程适用范围：已确认不限于 MJ 首版；任何已生成图片（关键帧、锚点等）用户不满意 / 效果不好 / 需要修改时进入精修分支。
8. 反馈记录：已确认写入 `.libtv/state.json`。
9. 精修节点位置：已确认直接放在原图节点旁边，不单独建组。
10. 默认保护约束：已确认核心强调“不得改动除用户指定修改点之外的任何画面区域”。
11. Step 2 锚点图：已确认复用该精修分支。
12. 多轮精修基准：已确认按用户反馈做状况识别；效果不好重新搞 → 用首版节点，基于当前轮继续 → 用当前轮精修节点。

详细流程见 [图片精修分支流程设计 v0.1](./2026-08-22-two-stage-image-refine-flow-design.md)。
