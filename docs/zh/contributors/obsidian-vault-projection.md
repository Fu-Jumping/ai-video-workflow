...
`project/` 仍是 AI 智能体工作目录和源文件根目录。`_views/obsidian/` 是 Obsidian vault 根目录。展开根目录后，先从 `00_开始审阅/` 进入；`01_阶段审核/` 是按阶段审核的主要观看层，`02_按镜头联查/` 只用于跨阶段异常定位，`03_审阅工具/` 放看板、交接、Canvas、Bases 和模板，`04_个人笔记/` 是用户自己的记录空间。用户应在 Obsidian 中打开 `_views/obsidian/`，不要打开 `project/` 本身。
...
- 使用编号标题的项目首页、阶段总览和审阅页
- 按 Step 目录生成、按镜头组和镜头顺序排列的阶段审核页
- 集中存放源文件路径、编辑边界和提示词的 `03_审阅工具/01_智能体交接.md` 页面
- 位于 `02_按镜头联查/单镜头/<shotId>.md` 的沉浸式跨阶段单镜头审阅页
- 带 Review Queue、Shot Progress、Execution Readiness、Modified Generated Files 视图的 Bases `.base` 文件
- 位于 `03_审阅工具/全局画布/` 的流程图、镜头流水线、审阅地图，以及位于 `02_按镜头联查/逐镜头审阅画布/` 的逐镜头审阅 Canvas 文件
- `投影清单.json` 生成清单
- `04_个人笔记/` 用户笔记入口
- 可选社区插件用法说明
...
当前观看层的主路线是“先阶段、后镜头”：从 `00_开始审阅/00_项目首页.md` 进入 `01_阶段审核/00_阶段总览.md`，在每个阶段按镜头组和镜头顺序从头到尾审核；只有发现跨阶段不一致时，才进入 `02_按镜头联查/`。

跨阶段联查页位于 `02_按镜头联查/单镜头/<shotId>.md`。它会链接和嵌入分镜、Step 4 图像提示词、Step 5 视频提示词、执行检查入口、用户审阅笔记目标和逐镜头审阅 Canvas。镜头页只保留短的修改入口，长期保留的人类评审记录应写在 `04_个人笔记/` 下。

`03_审阅工具/01_智能体交接.md` 集中提供可复制的智能体上下文、逐镜头源文件路径、编辑边界和验证命令。用户可以先在 Obsidian 中观看和定位问题，再进入交接页把对应内容复制给智能体。镜头审阅页不展开完整提示词，避免让面向智能体的文本淹没创作者审阅内容。

可选的 `--include-obsidian-ui` 建议会把项目首页、阶段总览、按镜头联查、制作看板、智能体交接、审阅地图、镜头流水线和个人笔记说明加入书签，并在工作区中打开开始审阅区与阶段审核区。
...
当前生成的观看层控制页、阶段审核页、单镜头页、智能体交接页和模板页使用显式编号标题，例如 `## 1. 打开路线`、`## 2. 审核顺序`、`## 5. 视频提示词` 和 `### 4.1 单镜头检查`。编号用于 Obsidian 大纲、页面内扫读和交接定位；`01_阶段审核/` 下投影出来的 Step 文件正文仍保持源文件原样，不会被自动加编号。

`01_阶段审核/` 投影页会把能映射到项目 Step Markdown 文件的源相对链接改写为 vault 内链接。例如步骤三镜头组文件中的 `../../04_图片提示词/镜头组-001/镜头-002-关键帧-01.md` 会指向 `01_阶段审核/04_图片提示词/镜头组-001/...` 下的真实投影文件，避免 Obsidian 点击时尝试创建不存在的路径。指向 Markdown 笔记的 Obsidian wiki link 使用无 `.md` 后缀的原生目标；Canvas 和 Base 链接仍保留 `.canvas` / `.base` 扩展名。
...
`04_个人笔记/` 是 Obsidian 内的用户补充空间，适合放评审记录、会议记录、研究笔记和临时想法。增量导出不会覆盖用户在该目录下新增的文件；从旧布局升级时，未跟踪的 `笔记/` 用户笔记会迁移到这里，目标冲突则保留源文件。源 Step 文件仍然是工作流事实源，Obsidian 笔记是辅助材料。
...
- 确认项目首页、阶段总览、按镜头联查、智能体交接、审阅地图、镜头流水线和个人笔记都容易进入。
