...
`流程/` 投影页会把能映射到项目 Step Markdown 文件的源相对链接改写为 vault 内链接。例如步骤三中的 `../04_图片提示词/镜头-002-关键帧.md` 会指向 `流程/步骤四 - 图片提示词/...` 下的真实投影文件，避免 Obsidian 点击时尝试创建不存在的路径。指向 Markdown 笔记的 Obsidian wiki link 使用无 `.md` 后缀的原生目标；Canvas 和 Base 链接仍保留 `.canvas` / `.base` 扩展名。
