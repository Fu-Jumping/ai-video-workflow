...
> 注：上图绘制于 Step 0 与 Step 7 并入之前的六步时期，图中步骤卡片仍为 Step 1-6 形态；当前正式流程为研究模式 Step 0 到 Step 7（剧本模式 Step 1 到 Step 7），以 [工作流总览](../workflow/overview.md) 与工作流主规范为准。
...
1. 顶部是用户使用项目的路径：安装、初始化、同步 IDE 运行规则、推进 Step 0-7 流程、校验诊断、交付或回流。
2. 左侧是产品与工具层：官方 pack、规则、skills、模板、CLI 和 checks。
3. 中间是 Step 0 到 Step 7 的创作与生产链路。每一步先展示输入、创作动作和审核门槛。
4. 步骤下方是独立的分步产物卡片。每个产物都用连线接回上方对应步骤，避免把“流程动作”和“交付物”挤在同一个框里。
...
| 落位 | 这个产物在项目目录里通常放在哪里 | 例如 `03_分镜脚本/` |
...
研究模式项目从 Step 0 前期研究起步（整理来源资料与创作简报），已有完整剧本的项目可直接从 Step 1 开始。每一步不是随手写一个文件，而是围绕一个稳定产物完成：先读取上游输入，再生成当前步骤内容，再按质量门槛审核，必要时回改。进入 Step 6 后，项目会把已启用上游步骤的产物转成可执行、可分发、可追踪的生产清单；成片进入发布准备时，Step 7 把故事内核与分镜高光转成各平台发布物料。
...
| --- | --- | --- | --- | --- |
| Step 0 前期研究 | 真实题材、新闻、采访、历史资料、评论样本 | 建立可追溯资料库与创作简报 | 关键现实事实必须带 `SRC-xxxx` 来源 ID | 研究总览、资料索引、摘录卡片、主题归纳、创作简报 |
| Step 1 策划 | 用户意图、题材、目标（研究模式继承 Step 0 创作简报） | 建立故事、视觉和生产基线 | 不越级写设定、分镜或 prompt | 故事内核、原始剧本、约束条件 |
...
| Step 6 执行方案 | 已启用上游步骤的完成态 | 汇总任务、缺口、阻塞、分发和放行条件 | 能追溯到 Step 3/4/5，不偷改上游创作正文 | 生产执行统筹单、图片执行单、视频执行单 |
| Step 7 发布物料 | Step 1 + Step 3（只读） | 把故事内核与分镜高光转成平台物料 | 不编造成片不存在的卖点，不宣传未锁版镜头 | 发布总表、标题、简介正文、话题标签、封面文案 |
...
Step 4 和 Step 5 的关系也很关键。Step 5 默认消费 Step 4 的 `可复制提示词`，但不能只写“沿用上一张图”。视频提示词必须重述当前段所需的连续性变量。
...
- Workflow pack：继续打磨分步规则、模板、状态回报和增强流程。
- IDE adapters：完善 Codex、Cursor、Claude Code、Trae 的同步策略和使用说明。
- Demo project：做一个可公开、可 verify、可从 Step 0 跑到 Step 7 的完整短片示例。
