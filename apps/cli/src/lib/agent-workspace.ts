...
export const generatedLocalSurfaceIgnoreBlock = [
  ...
  "00_前期研究/_资料库/*/raw/",
  "00_前期研究/_资料库/*/media/",
  "00_前期研究/_资料库/*/full-comments/",
  "00_前期研究/_资料库/*/browser-profile/",
  "00_前期研究/_资料库/*/cookies/",
  "00_前期研究/_inbox/raw/",
  "00_前期研究/_inbox/media/",
  "00_前期研究/_inbox/browser-profile/",
  "00_前期研究/_inbox/cookies/",
  "SOUL.md",
  "USER.md",
  "soul.md",
  "user.md",
...
export function sharedAgentEntryMergeBlock(): string {
  return [
    ...
    "- 将 `project-step-files` 理解为已启用步骤的创作事实源；研究模式包含 `00_前期研究/` 到 `07_发布物料/`，剧本模式包含 `01_概念策划/` 到 `07_发布物料/`。",
...
export const sharedAiWorkspaceDocs: Record<string, string> = {
  "入口说明.md": [
    ...
    "5. 源文件目录：研究模式为 `00_前期研究/` 到 `07_发布物料/`；剧本模式为 `01_概念策划/` 到 `07_发布物料/`",
    "6. `_views/obsidian/` 只作为生成的观看层，不作为源文件层",
    "",
    "## 事实源",
    "",
    "- `project-step-files` 表示当前项目已启用步骤的 Markdown 文件是创作事实源。",
  ...
  "智能体交接.md": [
    ...
    "## 修改路由",
    "",
    "- 真实资料、来源台账、摘录卡片、主题归纳和创作简报修改写入步骤零前期研究文件。",
  ...
  "入口协调.md": [
    ...
    "- `project-step-files` 是当前项目已启用步骤的创作事实源。",
    "",
    "## 本地和适配器表面",
    "",
...
export function sharedAgentEntryContent(): string {
  return [
    ...
    "## 对话引导",
    "",
    "- 每个实质性回复末尾以块引用附两行状态脚注：`> **【当前】** <位置与状态>` 与 `> **【下一步】** <具体动作>`，每行只说一件事。位置按所处步骤目录（`00_前期研究` 到 `07_发布物料`）、校验状态和偏离登记状态判断。",
    "- 所有创作工作都在本项目目录内进行；工具仓库只用于维护工作流本身。",
    "- CLI 命令（`verify`、`sync`、`doctor` 等）由全局安装的 `ai-video-workflow` 提供，以 `--project <本项目路径>` 指向本项目运行。",
    "- 进入或离开一个步骤前，提醒先运行对应的 `verify --step <N>`；校验通过再推进。",
    "- 校验失败先分类：内容问题引导用户回到出错的步骤源文件修复；属于合理流程偏离的（如轻量镜头不建关键帧），引导用 `deviation add` 或 `set-shot-mode` 显式登记（带 reason）后再继续，未登记的偏离不得静默绕过。",
    "",
    "## 全局规则",
    "",
    "- 保持步骤三和步骤四逐镜头对齐。",
