...
- 默认优先消费 Step 4 的 `可复制提示词`，必要时回读 `中文完整版本` 做审阅和消歧。
- 若 Step 4 的 `中文完整版本` 或 `可复制提示词` 包含 `@xx三视图` / `@xx场景图`，Step 5 必须继续写出同一组引用。
- Step 5 应以已通过关键帧 / 首帧图作为视频起点，同时用 `延续 @xx三视图、@xx场景图` 固定角色和场景一致性。
- 不用“沿用图片”“保持人物一致”替代明确的 `@xx三视图` / `@xx场景图`。
- 每份 Step 5 文件必须包含 `平台执行设置`，写清默认视频平台、输入方式、开场参考、时长上限、画幅和负面约束。
